...
const authEmail = u => u.trim().toLowerCase() + "@neodit.local";
...
function authForm(mode) {
  openModal(`<h2>${mode === "login" ? "Вход в Neodit" : "Регистрация"}</h2>
  <form id="authForm">
    <div class="field"><label>Юзернейм</label><input id="username" required minlength="3" maxlength="24" pattern="[A-Za-z0-9_]+"></div>
    <div class="field"><label>Пароль</label><input id="password" type="password" required minlength="6"></div>
    <div id="authError" class="error"></div>
    <button class="primary full">${mode === "login" ? "Войти" : "Создать аккаунт"}</button>
  </form>`);
  $("authForm").onsubmit = async e => {
    ...
    let result;
    if (mode === "signup") {
      result = await db.auth.signUp({ email: authEmail(username), password, options: { data: { username } } });
    } else {
      result = await db.auth.signInWithPassword({ email: authEmail(username), password });
    }
    if (result.error) {
    ...
    if (mode === "signup" && !result.data.session) {
      $("authError").textContent = "Регистрация создана. В Supabase отключи Authentication → Providers → Email → Confirm email, чтобы вход был сразу.";
      return;
    }
    user = result.data.user;
    await db.from("profiles").upsert({ id: user.id, username });
    await loadData();
    authUI();
    closeModal();
    toast("Готово!");
