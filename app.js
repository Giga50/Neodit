...
function authForm(mode) {
  ...
  $("authForm").onsubmit = async e => {
    ...
    if (isSignup) {
      const username = $("username").value.trim();
      const profileResult = await db.from("profiles").upsert({ id: user.id, username });
      if (profileResult.error) {
        $("authError").textContent = "Аккаунт создан, но профиль не сохранился: " + profileResult.error.message;
        return;
      }
    }
